...
The suite is green: **2,455 passing, 1 skipped** (2026-08-17, after the validate
split, the comment pass and the post-lock Merit work), with the one machine-dependent
test described immediately below — which PASSES on this machine.

⚠ The 1 SKIP is conditional and healthy, not a disabled test:
...
⚠ Quote the RUN's numbers, not `--collect-only`'s: the two have disagreed by one here
and the cause was not chased. The run is what tells you the suite is green.
...
| **Adversary roster — DONE, browser-verified** (GM-mode extras/beasts/NPCs; one small model that is NOT a Character; 52 generic templates; instancing) | `docs/status/adversary-roster.md` |

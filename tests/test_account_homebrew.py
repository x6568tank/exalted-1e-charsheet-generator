...
from . import _account_homebrew_state as state

MAIN = "tests/_account_homebrew_main.py"


@pytest.fixture(autouse=True)
def _no_default_library(monkeypatch):
    """The hosted switch, and a default library that must stay empty."""
    monkeypatch.setenv(custom_content.CUSTOM_DIR_ENV, str(state.DEFAULT_LIBRARY))
    custom_content.require_explicit_dir(True)
    yield
    custom_content.require_explicit_dir(False)
    assert not state.DEFAULT_LIBRARY.exists() or not any(state.DEFAULT_LIBRARY.rglob("*")), (
        f"A hosted action wrote into the default library {state.DEFAULT_LIBRARY}.")


async def _open(create_user) -> tuple[User, dict]:
    """Open the builder as a new browser. Return the user and its session context."""
...
@pytest.mark.asyncio
@pytest.mark.nicegui_main_file(MAIN)
async def test_a_hosted_party_save_uses_the_account_library(create_user) -> None:
    user, ctx = await _open(create_user)
    await user.open("/gm")
    user.find("gm-save-party").click()
    await user.should_see("Saved party")

    assert ctx["party_path"].is_file()

"""The Eclipse Caste generalist rule (core p.127).
...
`foreign_charm_xp_multiplier`, so the Abyssal Moonshadow parallel is a one-line
change to castes.json. The chargen half of the rule is
`Character.st_foreign_charms`.
...
def test_only_eclipse_carries_the_privilege(rs):
    assert rs.castes["eclipse"].foreign_charms is True
    assert rs.castes["eclipse"].foreign_charm_xp_multiplier == 2

...
# The destination
#
# This factory is the DESKTOP one. The hosted server gives each stored character
# its own file (`server/home.py`); `tests/test_character_pages.py` tests that.
# --------------------------------------------------------------------------- #

def test_the_desktop_keeps_the_prototype_path() -> None:
    """One user owns the file system, thus Save must write the file that the
    user opened. Do not "fix" it to isolate."""
    proto = _prototype()
